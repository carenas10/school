<?php
    ...
    if (mysqli_connect_errno($connection)) $json = array("sucess" => false, "reason" => mysqli_connect_error);
        ...
        if($login_row["password"] != $_GET["password"]) $json = array("sucess" => false, "reason" => "Bad username or password");
            ...
            $result = mysqli_query($connection,"SELECT * FROM datetimes where _id = \"" . $_GET["GID"] . "\"");
            $result = mysqli_fetch_array($result);
            if($result["user"] != $login_row["_id"]){
                $json = array("sucess" => false, "reason" => "You do not have permission to delete that datetime.");
            } else {
                $result = mysqli_query($connection,"UPDATE datetimes SET is_deleted='1' "
                . "WHERE _id = \"" . $_GET["GID"] . "\"");
