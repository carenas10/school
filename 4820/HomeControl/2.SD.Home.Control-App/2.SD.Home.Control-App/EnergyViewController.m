...
@property (strong, nonatomic) IBOutlet UILabel *currentEnergyLabel;

@end

@implementation EnergyViewController
    AppDelegate *energyAppDelegate;
    float setEnergy2=0;
    NSMutableArray *energyData;
    int numberOfViews_E=20;
...
-(void)adminCheck {
    ...
    if(isAdmin==1) {
        NSLog(@"isAdmin");
        energyAppDelegate.isAdmin = true;
    } else {
        energyAppDelegate.isAdmin = false;
        NSLog(@"isNotAdmin");
    }
    //end admin check
}
...
-(void)updateUI
{
    ...
    NSString *strURL = [NSString stringWithFormat:@"http://people.cs.clemson.edu/~jacosta/getLastPower.php"];
    NSError *error = nil;
    NSString *strResult;
    ...
    energyAppDelegate.currentPower = [strResult floatValue];
    self.currentLabel.text = [NSString stringWithFormat:@"%d", (int)[strResult integerValue]];
    
    //Get ideal humidity from DB
    NSString *strURL2 = [NSString stringWithFormat:@"http://people.cs.clemson.edu/~jacosta/getIdealPower.php"];
    NSError *error2= nil;
    NSString *strResult2;
    ...
    //Set ideal label
    energyAppDelegate.setPower = [strResult2 floatValue];
    self.idealLabel.text = [NSString stringWithFormat:@"Set: %d", (int)[strResult2 integerValue]];
    setEnergy2 = [strResult2 floatValue];
    [self setColorOfCurrentEnergy];
    
    //if user not admin, disable set button
    if(energyAppDelegate.isAdmin) {
    ...
    NSString *strURL1 = [NSString stringWithFormat:@"http://people.cs.clemson.edu/~jacosta/getAllPower.php"];
    NSError *error1 = nil;
    NSString *strResult3;
    ...
    NSRange foundRange;
    NSString *substring = @"currentUseinkWh=";
    ...
    while (searchRange.location < jsonData.length) {
        ...
        if (foundRange.location != NSNotFound) {
            ...
            if(secondRange.location !=NSNotFound)
            {
                ...
                [energyData addObject:[jsonData substringWithRange:NSMakeRange(startLocation, lengthOfNameString)]];
            }
        } else {
...
-(void)doSync
{
    energyAppDelegate.setHumidity = setEnergy2;
    
    NSString *strURL = [NSString stringWithFormat:@"http://people.cs.clemson.edu/~jacosta/setIdealPowerinkWh.php?setIdealPowerinkWh=%d", (int)setEnergy2];
    NSError *error = nil;
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Energy Set"
                                                    message:[NSString stringWithFormat:@"Your energy goal is now %d kWh", (int)energyAppDelegate.setHumidity]
                                                   delegate:nil
...
-(IBAction)setEnergyWithButton:(id)sender {
    
    //open dialog
    UIAlertView * setAlert = [[UIAlertView alloc] initWithTitle:@"" message:@"Enter new goal (in kWh):" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Ok", nil];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex == 1) {
        ...
        if ([self isPositiveNumber:input] && input.length <= 5) {
            setEnergy2 = [input floatValue];
            [self.idealLabel setText:[NSString stringWithFormat:@"Goal: %.1f",setEnergy2]];
            [self doSync];
        } else {
...
-(void)setColorOfCurrentEnergy
{
    //if current less than ideal, good.
    if(energyAppDelegate.currentPower < energyAppDelegate.setPower) {
...
- (void)loadView {
    [super loadView];
    energyData =  [[NSMutableArray alloc] initWithObjects:nil];
    energyAppDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    [self adminCheck];
    [self updateUI];
...
-(void)storeAllEnergyData
{
    self.chartData = [[NSMutableArray alloc] init];
    self.fullChartData = [[NSMutableArray alloc] init];
        // NSLog(@"count: %d", (int)energyData.count);
    for (int i = 0; i <energyData.count; i++) {
        
        if (i <= energyData.count) {
            NSString *currentEnergy = [NSString stringWithFormat:@"%@", energyData[i]];
            currentEnergy = [currentEnergy stringByReplacingOccurrencesOfString:@"\"" withString:@""];
            NSString *fullHumid = [currentEnergy stringByReplacingOccurrencesOfString:@"<br>" withString:@""];
            
            [self.fullChartData addObject:fullHumid];
            currentEnergy = [currentEnergy substringToIndex:5];
            currentEnergy = [currentEnergy stringByReplacingOccurrencesOfString:@"*" withString:@""];
                // NSLog(@"%@", currentEnergy);
            [self.chartData addObject:[NSNumber numberWithFloat:[currentEnergy doubleValue]]];
            if(i==energyData.count)
...
-(void)setDailyChart
{
    ...
    {
        // find how many hours a day the energy meter recorded data for
        ...
        numberOfViews_E = (int)averageOfHoursInDay.count;
...
- (IBAction)WeeklyChart:(id)sender {
    ...
    numberOfViews_E =7; // this can later be used for a user preference to expand to 10 day history instead
...
- (void)lineChartView:(JBLineChartView *)lineChartView
 didSelectLineAtIndex:(NSUInteger)lineIndex
    ...
    if(whichTypeofChart ==0) {
        ...
        self.currentEnergyLabel.text = [NSString stringWithFormat:@"%.0lf kWh", [[averageOfHoursInDay objectAtIndex:horizontalIndex] floatValue]];
    ...
    if(whichTypeofChart ==1) {
        self.currentEnergyLabel.text = [NSString stringWithFormat:@"%@ kWh", sevenData[(int)horizontalIndex]];
