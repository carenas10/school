...
@property (strong, nonatomic) IBOutlet UILabel *currentWaterLabel;

@end

@implementation WaterViewController
AppDelegate *waterAppDelegate;
float setwater2=0;
NSMutableArray *waterData;
int numberOfViews_W=20;
...
-(void)adminCheck {
    ...
    if(isAdmin==1) {
        NSLog(@"isAdmin");
        waterAppDelegate.isAdmin = true;
    } else {
        waterAppDelegate.isAdmin = false;
        NSLog(@"isNotAdmin");
    }
    //end admin check
}
...
-(void)updateUI
{
    ...
    NSString *strURL = [NSString stringWithFormat:@"http://people.cs.clemson.edu/~jacosta/getLastWater.php"];
    NSError *error = nil;
    NSString *strResult;
    ...
    waterAppDelegate.currentWater = [strResult floatValue];
    self.currentLabel.text = [NSString stringWithFormat:@"%d", (int)[strResult integerValue]];
    
        //Get ideal humidity from DB
    NSString *strURL2 = [NSString stringWithFormat:@"http://people.cs.clemson.edu/~jacosta/getIdealWater.php"];
    NSError *error2= nil;
    NSString *strResult2;
    ...
    strResult2 = [NSString stringWithContentsOfURL:[NSURL URLWithString:strURL2]  encoding:NSUTF8StringEncoding error:&error2];
        ...
        //Set ideal label
    waterAppDelegate.setWater = [strResult2 floatValue];
    self.idealLabel.text = [NSString stringWithFormat:@"Set: %d", (int)[strResult2 integerValue]];
    setwater2 = [strResult2 floatValue];
    [self setColorOfCurrentWater];
    
    //if user not admin, disable buttons
    if(waterAppDelegate.isAdmin) {
    ...
    NSString *strURL1 = [NSString stringWithFormat:@"http://people.cs.clemson.edu/~jacosta/getAllWater.php"];
    NSError *error1 = nil;
    NSString *strResult3;
    ...
    NSRange foundRange;
    NSString *substring = @"currentUseinGal=";
    ...
    while (searchRange.location < jsonData.length) {
        ...
        if (foundRange.location != NSNotFound) {
            ...
            if(secondRange.location !=NSNotFound)
            {
                ...
                [waterData addObject:[jsonData substringWithRange:NSMakeRange(startLocation, lengthOfNameString)]];
            }
        } else {
...
-(IBAction)setWaterWithButton:(id)sender {
    
    //open dialog
    UIAlertView * setAlert = [[UIAlertView alloc] initWithTitle:@"" message:@"Enter new goal water consumption:" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Ok", nil];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex == 1) {
        ...
        if ([self isPositiveNumber:input] && input.length <= 5) {
            setwater2 = [input floatValue];
            [self.idealLabel setText:[NSString stringWithFormat:@"Goal: %.1f",setwater2]];
            [self doSync];
        } else {
...
-(void)doSync
{
    waterAppDelegate.setWater = setwater2;
    
    NSString *strURL = [NSString stringWithFormat:@"http://people.cs.clemson.edu/~jacosta/setgoalWaterConsumption.php?setgoalWaterConsumption=%d", (int)setwater2];
    NSError *error = nil;
    ...
    returnString3 = [NSString stringWithContentsOfURL:[NSURL URLWithString:strURL]  encoding:NSUTF8StringEncoding error:&error];
        ...
        //won't need timer for set button stlyle.
        //[timer invalidate];
        //secondsE =0;
    
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Water Set"
                                                    message:[NSString stringWithFormat:@"Your water goal is now %d gal", (int)waterAppDelegate.setWater]
                                                   delegate:nil
                                          cancelButtonTitle:@"OK"
                                          otherButtonTitles:nil];
    [alert show];
...
-(void)setColorOfCurrentWater
{
        //if current less than ideal, good.
    if(waterAppDelegate.currentWater < waterAppDelegate.setWater) {
...
- (void)loadView {
    [super loadView];
    waterData =  [[NSMutableArray alloc] initWithObjects:nil];
    waterAppDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
        [self adminCheck];
    [self updateUI];
...
-(void)storeAllWaterData
{
    self.chartData = [[NSMutableArray alloc] init];
    self.fullChartData = [[NSMutableArray alloc] init];
        //NSLog(@"count: %d", (int)waterData.count);
    for (int i = 0; i <waterData.count; i++) {
        
        if (i <= waterData.count) {
            NSString *currentwater = [NSString stringWithFormat:@"%@", waterData[i]];
            currentwater = [currentwater stringByReplacingOccurrencesOfString:@"\"" withString:@""];
            NSString *fullHumid = [currentwater stringByReplacingOccurrencesOfString:@"<br>" withString:@""];
            
            [self.fullChartData addObject:fullHumid];
            currentwater = [currentwater substringToIndex:5];
            currentwater = [currentwater stringByReplacingOccurrencesOfString:@"*" withString:@""];
                // NSLog(@"%@", currentwater);
            [self.chartData addObject:[NSNumber numberWithFloat:[currentwater doubleValue]]];
            if(i==waterData.count)
...
-(void)setDailyChart
{
    ...
    {
            // find how many hours a day the water meter recorded data for
        ...
        numberOfViews_W = (int)averageOfHoursInDay.count;
            // NSLog(@"count: %d", numberOfViews_W);
...
- (IBAction)WeeklyChart:(id)sender {
    ...
    numberOfViews_W =7; // this can later be used for a user preference to expand to 10 day history instead
...
- (void)lineChartView:(JBLineChartView *)lineChartView
 didSelectLineAtIndex:(NSUInteger)lineIndex
    ...
    if(whichTypeofChart ==0) {
        ...
        self.currentWaterLabel.text = [NSString stringWithFormat:@"%.0lf gal", [[averageOfHoursInDay objectAtIndex:horizontalIndex] floatValue]];
    ...
    if(whichTypeofChart ==1) {
        self.currentWaterLabel.text = [NSString stringWithFormat:@"%@ gal", sevenData[(int)horizontalIndex]];
